...
con <- file("C:/Users/DELL/Documents/818_Project-main/results/models_output.log")
sink(con, append=TRUE)
sink(con, append=TRUE, type="message")

set.seed(1)

# df <- read.csv("C:/Users/DELL/Documents/5530_Project-main/data_raw/data_clean/Accidents_Sai.csv", stringsAsFactors = TRUE, nrows=300000)
df <- read.csv("C:/Users/DELL/Documents/818_Project-main/data_clean/Accidents_Sai.csv", stringsAsFactors = TRUE)
...
my_num_data <- train[, apply(train, is.numeric)]
...
lm_model <- lm(Severity ~ State + Traffic_Signal +Temperature + 
              Humidity... + Crossing + Visibility +  1, data = train)
...
x_num <- as.numeric(as.character(Severity_pred))            # Convert factor to numeric
x_num

ypredlm$mse <- (predlm$Severity - predlm$X_num)^2
...
#   fit(form, data=train)
rf <- tryCatch({get(load("C:/Users/DELL/Documents/818_Project-main/results/rfmodel.RData"))},
               error=function(cond) {randomForest(form, data=train, mtry=3, ntree=30)})
#mod_forest <- randomForest(form, data=train, mtry=3, ntree=30)

save(mod_forest, file="C:/Users/DELL/Documents/818_Project-main/results/rfmodel.RData")
...
accidents2 <- sample(1:nrow(df), 10000, replace = FALSE)
accidents2 <- df[accidents2, ]
set.seed(12345)
